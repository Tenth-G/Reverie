const routes = [
  ["/ugc/song/get?id=1", "GET"],
  ["/ugc/album/get?id=1", "GET"],
  ["/ugc/artist/get?id=1", "GET"],
  ["/ugc/artist/search?keyword=demo&limit=1", "GET"],
  ["/ugc/mv/get?id=1", "GET"],
  ["/ugc/detail?limit=1&offset=0", "GET"],
  ["/ugc/user/devote", "GET"],
];
...
(async () => {
  ...
  if (failures.length) {
    ...
    process.exitCode = 1;
  } else console.log("UGC route smoke test passed");
})();
