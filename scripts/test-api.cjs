...
async function main() {
  ...
  /* 1. search */
  const s = await req('/search?keywords=%E5%91%A8%E6%9D%B0%E4%BC%A6&limit=10&timestamp=' + Date.now())
  record('搜索接口 /search', s.status === 200 && (s.body?.result?.songs?.length || 0) > 0, `${s.body?.result?.songs?.length || 0} 首`)
  ...
  for (const id of ids) {
    const u = await req(`/song/url/v1?id=${id}&level=standard&timestamp=${Date.now()}`)
    if (u.body?.data?.[0]?.url) {
      freeSong = { id, url: u.body.data[0].url, br: u.body.data[0].br }
      break
    }
  }
  record('播放地址 /song/url/v1 (免费歌曲)', !!freeSong?.url, freeSong ? `id=${freeSong.id} br=${freeSong.br}` : '测试歌曲均需登录')

  /* 4. lyric */
  const l = await req('/lyric?id=186016&timestamp=' + Date.now())
  record('歌词接口 /lyric', !!(l.body?.lrc?.lyric), `lrc 长度 ${(l.body?.lrc?.lyric || '').length}`)
  ...
  /* 6. QR create */
  const c = await req(`/login/qr/create?key=${unikey}&qrimg=true&timestamp=${Date.now()}`)
  const qrimg = c.body?.data?.qrimg
  record('扫码-生成二维码 /login/qr/create', !!qrimg && qrimg.startsWith('data:image'), `base64 长度 ${(qrimg || '').length}`)

  /* 7. QR check (waiting state) */
  const chk = await req(`/login/qr/check?key=${unikey}&timestamp=${Date.now()}`)
  const code = chk.body?.code
  record('扫码-轮询 /login/qr/check', [800, 801, 802].includes(code), `code=${code} (801=等待扫码)`)

  /* 8. toplist + top/song */
  const t = await req('/toplist?timestamp=' + Date.now())
  record('排行榜 /toplist', (t.body?.list?.length || 0) > 0, `${t.body?.list?.length || 0} 个榜单`)
  const ts = await req('/top/song?type=0&timestamp=' + Date.now())
  record('歌曲榜 /top/song', (ts.body?.data?.length || 0) > 0, `${ts.body?.data?.length || 0} 首`)

  /* 9. playlist detail */
  const pl = await req('/playlist/detail?id=3778678&timestamp=' + Date.now())
  record('歌单详情 /playlist/detail', (pl.body?.playlist?.tracks?.length || 0) > 0, `${pl.body?.playlist?.name} · ${pl.body?.playlist?.tracks?.length || 0} 首`)
  ...
  record('登录状态 /login/status (未登录)', notLoggedIn === true, JSON.stringify({ code: ls.body?.code, data: ls.body?.data }).slice(0, 60))

  const rec = await req('/recommend/songs?timestamp=' + Date.now())
  record('每日推荐 /recommend/songs', rec.body?.code === 200, `code=${rec.body?.code} (未登录返回默认推荐，登录后个性化)`)
  ...
  require('fs').writeFileSync(
    ...
    JSON.stringify({ timestamp: new Date().toISOString(), total: results.length, passed, details: results }, null, 2),
  )
